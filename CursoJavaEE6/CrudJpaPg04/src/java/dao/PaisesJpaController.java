    ...
    public void create(Paises paises) throws RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Ciudades ciudadesCollectionCiudades : paises.getCiudadesCollection()) {
                Paises oldIdPaisesOfCiudadesCollectionCiudades = ciudadesCollectionCiudades.getIdPaises();
                ciudadesCollectionCiudades.setIdPaises(paises);
                ciudadesCollectionCiudades = em.merge(ciudadesCollectionCiudades);
                if (oldIdPaisesOfCiudadesCollectionCiudades != null) {
                    oldIdPaisesOfCiudadesCollectionCiudades.getCiudadesCollection().remove(ciudadesCollectionCiudades);
                    oldIdPaisesOfCiudadesCollectionCiudades = em.merge(oldIdPaisesOfCiudadesCollectionCiudades);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Ciudades ciudadesCollectionOldCiudades : ciudadesCollectionOld) {
                if (!ciudadesCollectionNew.contains(ciudadesCollectionOldCiudades)) {
                    ...
                    illegalOrphanMessages.add("You must retain Ciudades " + ciudadesCollectionOldCiudades + " since its idPaises field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Ciudades ciudadesCollectionNewCiudades : ciudadesCollectionNew) {
                if (!ciudadesCollectionOld.contains(ciudadesCollectionNewCiudades)) {
                    Paises oldIdPaisesOfCiudadesCollectionNewCiudades = ciudadesCollectionNewCiudades.getIdPaises();
                    ciudadesCollectionNewCiudades.setIdPaises(paises);
                    ciudadesCollectionNewCiudades = em.merge(ciudadesCollectionNewCiudades);
                    if (oldIdPaisesOfCiudadesCollectionNewCiudades != null && !oldIdPaisesOfCiudadesCollectionNewCiudades.equals(paises)) {
                        oldIdPaisesOfCiudadesCollectionNewCiudades.getCiudadesCollection().remove(ciudadesCollectionNewCiudades);
                        oldIdPaisesOfCiudadesCollectionNewCiudades = em.merge(oldIdPaisesOfCiudadesCollectionNewCiudades);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Ciudades ciudadesCollectionOrphanCheckCiudades : ciudadesCollectionOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Paises (" + paises + ") cannot be destroyed since the Ciudades " + ciudadesCollectionOrphanCheckCiudades + " in its ciudadesCollection field has a non-nullable idPaises field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
